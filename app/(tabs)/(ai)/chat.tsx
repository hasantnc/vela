...
import {
  ScrollView, View, Text, TextInput, KeyboardAvoidingView,
} from "react-native";
...
export default function AIScreen() {
  ...
  useEffect(() => {
    ...
    getDailyLimit(user.uid).then(setDailyLimit);
  ...
  const buildContext = () => {
    ...
    const txList = recent
      .slice(0, 10)
      .map((t) => `${t.emoji ?? ""} ${t.description ?? t.category} ${t.type === "expense" ? "-" : "+"}₺${t.amount}`)
      .join("\n");
...
${txList || "işlem yok"}`;
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `Sen VELA adlı bir kişisel finans uygulamasının Türkçe AI asistanısın. Kullanıcının gerçek harcama verilerine erişimin var. Kısa, samimi ve pratik cevaplar ver. Emoji kullan ama abartma. Türk lirası (₺) ile çalış. Finansal karakter analizi istendiğinde kullanıcının harcama alışkanlıklarını yorumla. Maksimum 3-4 paragraf yaz.`,
            },
            {
              role: "user",
      ...
      const reply =
        data?.choices?.[0]?.message?.content ?? "Üzgünüm, bir hata oluştu. Tekrar dener misin?";
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now() + 1,
          role: "assistant",
          content: "Bağlantı hatası oluştu. İnternet bağlantını kontrol et. 🔌",
        },
      ]);
    }
        ...
        >
          ...
          <View style={{
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 }}>
              ...
              <Text style={{ color: "#8B5CF6", fontSize: 11, fontWeight: "700", letterSpacing: 1, textTransform: "uppercase" }}>
            ...
            <View style={{ flexDirection: "row", gap: 16 }}>
              <View>
                ...
                <Text selectable style={{ color: "#34D399", fontSize: 14, fontWeight: "700" }}>
                  ₺{totalIncome.toFixed(0)}
                </Text>
              </View>
              <View>
                <Text style={{ color: "#555", fontSize: 11 }}>Bu ay gider</Text>
                <Text selectable style={{ color: "#F87171", fontSize: 14, fontWeight: "700" }}>
          ...
          {messages.map((msg) => (
            <View key={msg.id} style={{ alignItems: msg.role === "user" ? "flex-end" : "flex-start", gap: 4 }}>
              ...
              {msg.role === "user" && (
                <Text style={{ color: "#2D2D45", fontSize: 10 }}>Sen</Text>
              )}
            </View>
          ))}
          ...
          {loading && (
            <View style={{ alignItems: "flex-start", gap: 4 }}>
              ...
              <View style={{
                ...
                {[0, 1, 2].map((i) => (
                  <View key={i} style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: "#6D28D9", opacity: 0.7 + i * 0.1 }} />
                ))}
              </View>
            </View>
          )}
          ...
          {messages.length === 1 && (
            <View style={{ gap: 8, marginTop: 8 }}>
              <Text style={{ color: "#555", fontSize: 12, fontWeight: "600", marginBottom: 4 }}>
                Hızlı sorular:
              </Text>
              {QUICK_PROMPTS.map((p) => (
                <HapticPressable
                  ...
                  style={({ pressed }) => ({
                    backgroundColor: pressed ? `${p.color}14` : "rgba(255,255,255,0.04)",
                    borderWidth: 1,
                    borderColor: pressed ? `${p.color}33` : "rgba(255,255,255,0.09)",
                    borderRadius: 14,
                    paddingHorizontal: 14,
                    paddingVertical: 12,
                    flexDirection: "row",
        ...
        <View style={{
          ...
          {!isPremium && (
            <View style={{
              ...
              backgroundColor: aiUsedToday >= FREE_AI_LIMIT ? "rgba(239,68,68,0.1)" : "rgba(139,92,246,0.08)",
              borderWidth: 1,
              borderColor: aiUsedToday >= FREE_AI_LIMIT ? "rgba(239,68,68,0.25)" : "rgba(139,92,246,0.2)",
              ...
              <Text style={{ flex: 1, color: aiUsedToday >= FREE_AI_LIMIT ? "#F87171" : "#A78BFA", fontSize: 12, fontWeight: "600" }}>
                {aiUsedToday >= FREE_AI_LIMIT
                  ? "Günlük 3 soru hakkını kullandın — Premium'a geç"
                  : `Bugün ${FREE_AI_LIMIT - aiUsedToday} soru hakkın kaldı`}
              </Text>
              {aiUsedToday >= FREE_AI_LIMIT && (
          ...
          <View style={{ flexDirection: "row", gap: 10, alignItems: "flex-end" }}>
            ...
            <HapticPressable
              ...
              style={{
                ...
                backgroundColor: input.trim() && !loading ? "#8B5CF6" : "rgba(255,255,255,0.06)",
                alignItems: "center", justifyContent: "center",
              }}
            >
              <Ionicons
